...
from utils.nn_utils import BatchEdgeConstructor,BatchEdgeConstructor1, _knn_edges, print_cuda_memory,_radial_edges
...
class RadialBatchEdgeConstructor(BatchEdgeConstructor1):
    def __init__(self, cut_off, global_message_passing=True, global_node_id_vocab=[], delete_self_loop=True) -> None:
    ...
    def _construct_intra_edges(self, S, batch_id, segment_ids, **kwargs):
        ...
        intra_edges = _radial_edges(
            dist, src_dst, self.cut_off)
        return intra_edges
...
class KNNBatchEdgeConstructor1(BatchEdgeConstructor1):
...
class BlockEmbedding(nn.Module):
    ...
    def __init__(self, num_block_type, num_atom_type, num_atom_position, embed_size, no_block_embedding=False):
        ...
        if not no_block_embedding:
            self.block_embedding = nn.Embedding(num_block_type, embed_size) # embed_size 128
