...
# from nn_utils import stable_norm
...
class GET(nn.Module):
    ...
    def __init__(self, d_hidden, d_radial, n_channel, n_rbf, cutoff=7.0, d_edge=0, n_layers=4, n_head=4,
                 act_fn=nn.LeakyReLU(), residual=True, dropout=0.1, z_requires_grad=True, pre_norm=False,
                 sparse_k=3): #act_fn=nn.SiLU(),nn.ReLU()
        super().__init__()
        '''
    ...
    def recover_scale(self, Z, block_id, batch_id, record_scale):
        ...
        Z_c = scatter_mean(Z, unit_batch_id, dim=0)  # [bs, n_channel, 3] 计算一个item中所有原子坐标的均值
    ...
    def forward(self, H, Z, block_id, batch_id, edges, edge_attr=None, cached_unit_edge_info=None):
        ...
        for i in range(self.n_layers):
            ...
            # H_ffn=H # 变 去掉drop out 不变
            # Z_ffn=Z # 不变
            H, Z, rescale = self._modules[f'layernorm1_{i}'](H, Z, block_id, batch_id) # H,Z,rescale变
            record_scale *= rescale # 变
...
class GETLayer(nn.Module):
    ...
    def __init__(self, d_hidden, d_radial, n_channel, n_rbf, cutoff=7.0,
                 d_edge=0, n_head=4, act_fn=nn.LeakyReLU(), residual=True):
    ...
    def attention(self, H, Z, edges, edge_attr, cached_unit_edge_info):
        ...
        # beta_scatter_softmax=beta
        beta = beta[block_edge_id[unit_edge_src_start]].unsqueeze(-1)  # [Em, n_head, 1], Em = \sum_{i, j \in E} n_i
        # beta_select=beta[block_edge_id[unit_edge_src_start]]# gpu变 cpu 不变
        # block_uni=block_edge_id[unit_edge_src_start] #不变
    ...
    def invariant_update(self, H_v, H, alpha, beta, D, cached_unit_edge_info):
        ...
        #unit_col 不变
        H_v = self.unit_msg_mlp(torch.cat([H_v[unit_col], D], dim=-1))  # [Eu, n_head, d_radial / n_head] 变
        ...
        # H_agg_scatter_sum2=H_agg #变
        #unit_agg_row 不变
        H_agg = H_agg.view(H_agg.shape[0], -1)  # [N, hidden_size]
        H_agg = self.node_out_linear(H_agg) # 变
    ...
    def forward(self, H, Z, block_id, edges, edge_attr=None, cached_unit_edge_info=None):
        ...
        alpha, beta, (D, R, dZ)= self.attention(H, Z, edges, edge_attr, cached_unit_edge_info) #alpha D,dZ 不变, beta变
        
        H_v = self.linear_v(H.view(H.shape[0], self.n_head, -1))  # [N, n_head, d_radial / n_head] 不变
   
        H = self.invariant_update(H_v, H, alpha, beta, D, cached_unit_edge_info)# 变

        Z = self.equivariant_update(H_v, Z, alpha, beta, D, dZ, cached_unit_edge_info) # 变

        return H, Z


class EquivariantFFN(nn.Module):
    def __init__(self, d_in, d_hidden, d_out, n_channel, n_rbf=16, act_fn=nn.LeakyReLU(),
                 residual=True, dropout=0.2, constant=1, z_requires_grad=True) -> None:
        super().__init__()
...
class EquivariantLayerNorm(nn.Module):
    ...
    def __init__(self, d_hidden, n_channel, n_rbf=16, cutoff=7.0, act_fn=nn.LeakyReLU()) -> None:
        super().__init__()
    ...
    def forward(self, H, Z, block_id, batch_id):
        ...
        # H = self.layernorm(H)
        Z_c = scatter_mean(Z, unit_batch_id, dim=0)  # [bs, n_channel, 3] 用所有原子坐标计算整个图的centroid
        Z_c = Z_c[unit_batch_id]  # [N, n_channel, 3]
        Z_centered = Z - Z_c
        var = scatter_std(
