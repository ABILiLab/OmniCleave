...
class BatchEdgeConstructor:
    ...
    def get_batch_edges(self, batch_id):
        ...
        lengths = scatter_sum(torch.ones_like(batch_id), batch_id)  # [bs] tensor([297, 638]) batch中每个sample的长度
        N, max_n = batch_id.shape[0], torch.max(lengths) # 935 638
    ...
    def _prepare(self, S, batch_id, segment_ids) -> None:
        ...
        row_global, col_global = is_global[row], is_global[col] # 找到与全局节点相连的边
        not_global_edges = torch.logical_not(torch.logical_or(row_global, col_global)) # 找到与全局节点不相连的边
...
class BatchEdgeConstructor1:
    ...
    def _prepare(self, S, batch_id, segment_ids) -> None:
        (row, col), (offsets, max_n, gni2lni) = self.get_batch_edges(batch_id) # 先获取batch 每个图，所有可能的边，然后在后续步骤中改为KNN 边
    ...
    def _construct_intra_edges(self, S, batch_id, segment_ids, **kwargs):
        row, col = self.row, self.col
        # all possible ctx edges: same seg, not global
        select_edges = torch.logical_and(self.row_seg == self.col_seg, self.not_global_edges) #选择与全局节点不相关的边
        intra_all_row, intra_all_col = row[select_edges], col[select_edges]
        return torch.stack([intra_all_row, intra_all_col])
    ...
    def __call__(self, S, batch_id, segment_ids, **kwargs):
        ...
        # return intra_edges, inter_edges, global_normal_edges, global_global_edges, seq_edges
        return intra_edges,  global_normal_edges, global_global_edges, seq_edges
